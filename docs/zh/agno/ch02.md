...
> 本章目标：Set up可复用的开发环境，Run through第一个接入 DeepSeek 的 Agent，并Learn to troubleshootthree most common连接错误。
...
观察输出你会发现多了一段工具调用记录：模型先请求调用搜索工具，Agno 执行后把结果回传给模型生成最终答案——这正是第 1 章讲的运行循环。工具的原理与since定义方法在第 7、8 章展开。
...
const quiz = [
  ...
  },
  {
    ...
    answer: 1,
    explain: '该选项控制是否在响应里打印工具调用记录，便于调试和理解 Agent 行为；不影响工具能否被调用。'
  },
  {
...
3. 把 2.5 的 Agent 换成一个你感兴趣的中文问题（如"最近的 Python 版本有什么新特性"），to比加工具前后答案的质量差异。
