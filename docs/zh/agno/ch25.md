...
配合第 12 章的 `Router`/`Condition`，就能实现"小模型先分类、大模型只接难活"的漏斗结构。Team 中还可以只给 leader 配大模型，成员继承或显式用小模型。

## 25.2 并发吞吐：asyncio.gather + arun
...
Workflow 层面则用 `Parallel` 步骤自动并发（见第 12 章）。注意并发上限要与服务商限流匹配——用 `asyncio.Semaphore` 控制同时在飞的请求数。

## 25.3 成本控制四板斧

1. **分级路由**（见 25.1）——通常能砍掉 50% 以上成本；
...
1. 为第 11 章的写作流水线接入 `AccuracyEval`：准备 5 个主题与预期要点，跑通一次评估并接入 GitHub Actions。
...
> 🎉 生产实践篇完成！接下来是三章综合实战：从[第 26 章：实战一 · 智能数据分析助手](./ch26)开始，把前面所有能力拧成一股绳。
