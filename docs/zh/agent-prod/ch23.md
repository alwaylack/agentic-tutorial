...
> 📌 本章综合了本课程的多个章节：**FastAPI 生产基础**（[FastAPI ch22](/zh/fastapi/ch22)）、**AI SDK Provider 管理**（[AI SDK ch03](/zh/ai-sdk/ch03)）、**LangGraph 流式输出**（[Agent 工程 ch08](/zh/agent/ch08)）、**可观测性**（Langfuse（即将发布））。如遇到不熟悉的知识点，可回看对应章节。
...
3. **性能压测**：使用 Locust（参考 [Locust 教程](/zh/locust/)）测试 Agent 服务的并发能力
...
- [FastAPI 生产部署](/zh/fastapi/ch22) — Docker 与部署
- [AI SDK Provider 管理](/zh/ai-sdk/ch03) — Gateway 与自定义 Provider
- [LangGraph 持久化](/zh/agent/ch07) — Checkpointer 与 Thread
- [Langfuse 可观测性](/zh/agent-prod/)（即将发布） — 追踪与评估
