...
1. **异步化**：OCR 和 Whisper 都是 CPU/GPU 密集操作，在 FastAPI 中必须丢进 `run_in_executor` 或 Celery 任务队列（见 ch02），避免阻塞事件循环。
2. **幂等入库**：对文件内容算 SHA-256 作为文档 ID，重复上传直接跳过，防止向量库重复灌入（呼应 ch03 的幂等设计）。
3. **解析质量监控**：给每个解析任务记录 `char_count`、`ocr_confidence` 等指标上报到 Langfuse/CloudWatch（见 ch14/ch18），空解析率突增说明上游格式变了。
...
- **生产四件套**：异步化、幂等入库、质量监控、失败降级

## 🛠️ 动手实践

1. **解析质量评估集**：找 3 份不同类型的 PDF（原生/扫描/带表格），编写 pytest 用例断言解析输出的最小字符数与表格行数，把「文档解析」纳入 CI（呼应 pytest 课程 ch19）。
2. **元数据检索过滤**：扩展 `store_node`，把 `kind`、文件名、页码写入向量库 metadata，并在检索时支持 `filter={"kind": "pptx"}`。
3. **批量灌库脚本**：写一个 CLI 脚本遍历 S3 桶（boto3，复用 ch16 的凭证配置），把所有 PDF/PPTX 增量灌入知识库，用文件哈希实现幂等。
