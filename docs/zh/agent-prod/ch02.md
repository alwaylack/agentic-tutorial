...
> - 掌握 Celery 重试策略：`retry_backoff` 与 `max_retries`
...
在前一章中我们用 FastAPI 的 `BackgroundTasks` 处理轻量后台任务（参见 [FastAPI ch16](/zh/fastapi/ch16.md)）。但当任务需要：
...
1. **Redis 队列原语**：`RPUSH`/`LPOP` 实现 FIFO，`BRPOP` 实现阻塞消费，避免忙轮询；
...
6. **重试策略**：`retry_backoff=True` 启用指数退避，`max_retries` 限制最大尝试次数。

> 📌 更高级的异步交互模式（WebSocket 实时推送结果、Server-Sent Events）在 [LangGraph ch08](./ch08.md) 中讲解，本章先掌握轮询模式即可。
...
2. 在 `generate_agent_response` Task 中加入模拟 `RateLimitError`（前 3 次调用抛异常，第 4 次成功），观察 Celery Worker 日志中的自动重试行为，记录每次重试的时间间隔，验证指数退避是否生效。
...
const quiz = [
  ...
  {
    q: 'Celery 的 retry_backoff=True 配合 bind=True 时，重试间隔如何变化？',
    options: [
      '每次固定等待 1 秒',
      '每次固定等待 5 秒',
      '指数退避：第 n 次重试等待 2^(n-1) 秒（基准值默认为 2）',
      '随机等待 1~10 秒'
    ],
    answer: 2,
    explain: 'retry_backoff=True 启用指数退避，默认基准为 2 秒，即第1次等2s、第2次等4s、第3次等8s，避免对限流 API 的连续冲击。'
  },
  {
    q: 'AsyncResult.get() 阻塞等待结果时有什么风险？',
    options: [
      '会消耗 Redis 内存',
      '占用 HTTP 连接，高并发下可能导致服务撑不住',
  ...
  },
  {
    ...
    options: [
      '发送一封欢迎邮件（一次性，无持久化需求）',
      '写入访问日志（写文件即可）',
      'LLM 调用任务，需要重试、持久化和跨进程执行',
      '格式化一个请求参数（<1ms 即可完成）'
    ],
    answer: 2,
    explain: 'BackgroundTasks 与主进程同生共死，无法持久化或水平扩展；Celery Worker 独立进程，支持任务持久化、失败重试和多 Worker 水平扩展。'
  }
];
</script>
