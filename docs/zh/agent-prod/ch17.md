...
本章是生产部署的关键一环。前面的章节（[FastAPI ch22](/zh/fastapi/ch22) 讲了 Docker 基础）已经覆盖了容器化，这里我们讨论"容器之外"的基础设施——让 Agent 服务能够异步、可靠、水平扩展地运行。
...
前端通过 [FastAPI](/zh/fastapi/) 暴露一个 HTTP 接口把任务丢进 SQS，Worker 在另一个容器/进程中异步消费：
...
> 🔗 部署细节（Dockerfile、多容器编排、进程管理）参见 [FastAPI ch22](/zh/fastapi/ch22)。
