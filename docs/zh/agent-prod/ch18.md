...
CloudWatch Logs 是 AWS 原生的日志服务。要让日志有价值，必须**结构化**——否则就是一堆无法搜索的文本。
...
**注意**：生产环境推荐使用 [AWS Distro for OpenTelemetry (ADOT)](https://aws.amazon.com/opentelemetry/) 或 [CloudWatch Agent](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Install-CloudWatch-Agent.html)，它们支持自动批处理与重试。
...
- **结构化日志**：使用 JSON 格式输出，包含 `request_id`、`latency_ms`、`error_code` 等字段
...
const quiz = [
  ...
  {
    question: "GitHub Actions 中，如何实现只在 main 分支部署？",
    options: [
    ...
    answer: 1,
    explanation: "通过在 job 或 step 中添加 `if: github.ref == 'refs/heads/main'` 条件，可以精确控制仅在 main 分支触发部署。"
  },
  {
    question: "结构化日志的主要优势是什么？",
    options: [
      "减少日志存储空间",
      "便于程序解析和查询",
      "提高日志写入速度",
    ...
    answer: 1,
    explanation: "结构化日志（如 JSON）可以被程序直接解析，便于在 CloudWatch Logs Insights 中使用 parse、stats 等关键字进行查询和分析。"
  }
];
</script>
...
> 📌 本章内容假设你已经完成 [FastAPI ch22](/zh/fastapi/ch22) 的 Docker 基础，本章在此基础上讲述如何将其部署到 AWS 并加上监控与 CI/CD。
