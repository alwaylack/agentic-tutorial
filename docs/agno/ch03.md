...
这种写法的问题显而易见：提示词越来越长、token 成本线性上涨、无法持久化。所以它只适合理解原理，生产环境请使用第 14 章的会话机制。
...
- 手动拼历史能实现多轮但成本高，正确方案是会话存储 + chat history（第 14–15 章）。
...
> 掌握了同步运行后，进入[第 4 章：流式输出与实时响应](./ch04)。
