# 第 14 章 · 会话管理 session state 与 chat history
...
| Memory（用户记忆） | 提炼出的用户事实 | `user_id`（跨会话） | 偏好、画像（第 16 章） |

一句话总结：**history 是"聊过什么"，state 是"办到哪了"，memory 是"这人是谁"**。本章聚焦前两者。

## 14.2 session_id 与聊天历史
...
1. 把 14.3 的购物清单扩展出 `remove_item(item: str)` 工具，测试"把鸡蛋去掉"能否正确删除，并观察 instructions 中 `{shopping_list}` 的实时变化。
...
> 完成动手实践后，进入[第 15 章：存储 Storage——会话持久化](./ch15)。
