...
分工原则与第 21 章一致：**确定性的活给函数执行器（零 token），需要推理的活才交给 Agent**。数据探查是固定动作（describe + 维度枚举），没必要烧模型；而"哪里的解读值得写进报告"必须由模型判断。
...
::: tip 为什么返回路径而不是图片内容
模型无法"看"本地 PNG，但报告里需要引用它。返回 `saved_to` 路径让模型在 Markdown 里写出 `![...](charts/region_amount.png)` 即可；若要真正视觉理解图片，需走多模态输入（第 16 章），那是另一个场景。
:::
...
注意 `probe` 的签名约定：接收 `StepInput`、返回 `StepOutput`，通过 `previous_step_content` 在步骤间传递数据（第 11 章的模式）。整条流水线只有 Step2 会消耗 token，且它的起点已经是结构化的数据画像——比裸问模型省得多。

## 26.6 可选增强：Team 双角色打磨报告

如果报告质量要求高（对外发布级别），可以把 Step2~Step3 换成一个 **Team**：分析师产出初稿，审稿人逐条核对数字后定稿。Team 的路由模式让审稿意见自动回传（详见第 19 章）：
...
实战取舍建议：内部周报用 26.5 的线性 Workflow（快、便宜、稳定）；对外发布再上 Team（慢一点，但数字经过核验）。**不要默认全上 Team**——多一个成员就多几轮交互 token。
...
2. 把 26.5 流水线的 Step3 输出落盘为 `report.md` 文件，并用 Python 定时任务（`schedule` 库）模拟每周一自动生成周报。
3. 参考 26.6 把 `fact_checker` 加入现有 Workflow 作为第四个 Step，对比加入前后报告中的数字错误率，写下你的取舍结论。

> 把这套"探查 → 分析 → 撰写"骨架换成你的业务数据试试。下一章我们把多路检索交给 Team 协作——[第 27 章：多源研究助手 Team](./ch27)。
