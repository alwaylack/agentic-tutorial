...
2. 把 13.5 节的发票提取改造成合同关键信息提取（甲乙方、金额、期限），设计你自己的 Pydantic 模型。
3. 给 13.4 的生图 Agent 加一个 `save_last_image()` 自定义工具，让用户能用一句"把刚才的图存下来"完成落盘。

> 完成动手实践后，进入[第 14 章：会话管理 session state 与 chat history](./ch14)。
