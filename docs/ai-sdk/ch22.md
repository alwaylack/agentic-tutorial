...
<script setup>
const quiz = [
  { q: '客户端发到服务端的 messages 是什么类型？', options: ['ModelMessage[]', '纯字符串数组', 'UIMessage[]（含 parts 的结构化消息）', 'OpenAI 原始格式的 JSON'], answer: 2, explain: 'AI SDK v6 前后端统一使用 UIMessage 作为传输格式，服务端再经 convertToModelMessages 转换。' },
