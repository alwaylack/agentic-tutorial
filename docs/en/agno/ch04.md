...
在 FastAPI 中，把 `async for` 里收到的片段通过 `StreamingResponse` 或 WebSocket 推给前端，就实现了网页版打字机——这是第 22 章（结合 FastAPI）的实战预告。
...
2. 用 `stream_events=True` 跑一个带搜索工具的问题，把所有事件按顺序写入 `events.log` 文件，观察事件出现的先后规律。
