...
> Chapter goals：Master Team 的四种协作模式切换、成员间共享 session state 的正确姿势，以及如何通过流式事件观察团队内部分工。

## 10.1 四种协作模式总览

上一章我们用 `mode=TeamMode.coordinate` 让 leader 分派任务。Agno 把协作风格显式化为四种模式（`from agno.team.mode import TeamMode`）：
...
> 下一章我们把"确定性"请回来：当流程不该由模型临场发挥时，[Workflow](./ch11) 是更好的编排Tool。
