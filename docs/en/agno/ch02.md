...
> Chapter goals：搭好可复用的开发环境，跑通第一个接入 DeepSeek 的 Agent，并学会排查三类最常见的连接错误。
...
news_agent = Agent(
    ...
    markdown=True,
    show_tool_calls=True,                 # 在输出中展示tool calling过程
)

news_agent.print_response("今天有哪些关于 AI 的新闻？", stream=True)
```

观察输出你会发现多了一段tool calling记录：模型先请求调用搜索工具，Agno 执行后把结果回传给模型生成最终答案——这正是第 1 章讲的运行循环。工具的原理与自定义方法在第 7、8 章展开。
...
const quiz = [
  ...
  },
  {
    ...
    options: ['禁止模型调用工具', '在输出中展示tool calling的详细过程', '自动retry失败的tool calling', '缓存工具结果'],
    answer: 1,
    explain: '该选项控制是否在响应里打印tool calling记录，便于调试和Understand Agent 行为；不影响工具能否被调用。'
  },
  {
  ...
  },
  {
    ...
    answer: 2,
    explain: '模型自身无法联网，必须通过注入工具（如 DuckDuckGoTools）赋予其检索能力；temperature 和 markdown 都与联网无关。'
  }
]
</script>
...
> 能稳定跑通带工具的 Agent 后，进入[第 3 章：运行 Agent 与 RunOutput 解析](./ch03)。
