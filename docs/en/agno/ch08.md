# Chapter 8 · 自定义工具与 Toolkit 开发

> Chapter goals：Master三种自定义工具的写法——普通函数、`@tool` 装饰器、Toolkit 子类，学会用装饰器参数控制工具行为，并建立正确的工具设计粒度观。

## 8.1 普通函数：最轻量的自定义工具

上一章已经用过普通函数作工具，这里补全工程细节。函数工具支持同步与async两种定义：

```python
# custom_func.py —— 同步/async函数皆可作工具
import asyncio
import os
import sqlite3
...
工具会访问外部世界，失败是常态。Agno 会捕获工具抛出的异常并把错误信息回传给模型，让模型决定retry还是换路；你也可以通过 `tool_hooks` 做统一兜底：
...
推荐的做法是**面向任务建模**：一个工具完成一件模型视角下完整的事，参数尽量少而明确。对比：
...
1. **幂等与副作用分离**：只读工具随便调；有副作用的工具（下单、发邮件）务必配 `requires_confirmation=True` 或rate limiting；
2. **超时与retry在工具内部解决**：不要指望模型自己处理网络抖动。
...
const quiz = [
  ...
  },
  {
    ...
    answer: 2,
    explain: '框架会捕获工具异常并把错误内容加入上下文，模型可以据此调整参数retry或换一种回答方式；开发者可用 hooks 统一加工错误文案。'
  },
  {
...
> 完成动手实践后，进入[第 9 章：Agent Team 入门](./ch09)——单 Agent 之外的multi-agentcollaboration世界。
