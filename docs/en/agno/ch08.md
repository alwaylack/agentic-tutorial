...
上一章已经用过普通函数作Tool，这里补全工程细节。函数Tool支持同步与异步两种定义：

```python
# custom_func.py —— 同步/异步函数皆可作Tool
import asyncio
import os
import sqlite3
...
::: tip 返回值约定
Tool可以返回字符串、数字、字典、列表等可序列化值。返回**结构化数据**（dict/list）通常比拼接大段文本更好——模型能更准确地提取所需字段。若需要随文本一起返回媒体文件，可使用 `ToolResult`。
:::
...
| 参数 | 默认 | 说明 |
|---|---|---|
| `name` / `description` | None | 覆盖模型可见的Tool名/描述 |
...
当一个业务域有多个操作且需要共享状态（如统一的客户端、鉴权配置），就该写 Toolkit 了。写法是继承 `agno.tools.Toolkit` 并Implement `__init__`：
...
Tool会访问外部世界，失败是常态。Agno 会捕获Tool抛出的异常并把错误信息回传给模型，让模型决定重试还是换路；你也可以通过 `tool_hooks` 做统一兜底：
...
> 经验法则：错误信息写给"人"和写给"模型"要分开设计。给模型的错误要说清楚**下一步能做什么**（"请检查参数""该城市暂无数据，请换一个城市"），而不是堆一段堆栈。

## 8.5 Tool设计的粒度原则

初学者最容易犯的两个极端：

- ❌ **粒度太细**：`get_id`、`get_name`、`get_email` 三个Tool，模型要多轮调用拼装；
- ❌ **粒度太粗**：一个 `manage_company(query: str)` 万能接口，schema 含糊导致模型乱传参。

推荐的做法是**面向任务建模**：一个Tool完成一件模型视角下完整的事，参数尽量少而明确。对比：
...
1. **幂等与副作用分离**：只读Tool随便调；有副作用的Tool（下单、发邮件）务必配 `requires_confirmation=True` 或限流；
2. **超时与重试在Tool内部解决**：不要指望模型自己处理网络抖动。
...
- Tool异常会被 Agno 捕获回传给模型，错误文案要写成"可行动"的指引；
...
const quiz = [
  {
    question: '某Tool用于"提交订单"，希望调用完成后立即结束本次运行，应使用哪个参数？',
    options: ['cache_results=True', 'stop_after_tool_call=True', 'show_result=True', 'external_execution=False'],
    answer: 1,
    explain: 'stop_after_tool_call=True 表示该Tool执行完毕后终止本轮 run，适合提交类"终态动作"，可省去模型额外的一轮总结调用。'
  },
  {
    ...
    answer: 1,
    explain: 'Toolkit 通过 register() 把实例方法登记为可被模型调用的Tool；未注册的方法只是普通 Python 方法，不会进入Tool schema。'
  },
  {
    ...
    answer: 2,
    explain: '框架会捕获Tool异常并把错误内容加入上下文，模型可以据此调整参数重试或换一种回答方式；开发者可用 hooks 统一加工错误文案。'
  },
  {
    ...
    answer: 2,
    explain: '好的Tool一次调用完成一件完整的事并返回结构化结果。field 字符串靠猜、裸 SQL 万能接口、过度拆分都会增加模型出错概率与调用轮数。'
  }
]
</script>

1. 为 8.3 节的 `TodoTools` 增加 `clear_finished(done_index: int)` 方法并注册，验证模型能用自然语言正确传入"第几项"。
2. 给某个模拟耗时 3 秒的行情Tool分别开启/关闭 `cache_results`，连续提问两次，用日志对比两次运行的响应差异。
...
> 完成动手实践后，进入[第 9 章：Agent Team 入门](./ch09)——单 Agent 之外的多智能体协作世界。
