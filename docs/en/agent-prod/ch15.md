...
> Chapter goals：
> - Master多 Provider 路由策略
> - Implement基于任务复杂度的智能路由
> - 设计成本monitoring和预算告警
> - Implement故障降级机制
...
1. Implement三档路由（简单/中等/复杂）映射到不同模型
2. 添加预算告警，超支时自动切换到低成本模型
3. Implement主备降级，主模型超时 5s 自动切备用
...
const quiz = [
  ...
  {
    q: '成本monitoring的核心目的是？',
    options: ['加速响应', '防止预算超支', '提高准确率', '减少 token'],
    answer: 1,
    explain: 'monitoring确保不超出预设的成本预算。'
  },
  {
    q: '故障降级的触发条件通常是？',
    options: ['成本过高', '超时或错误', 'concurrency过高', '模型更新'],
    answer: 1,
    explain: '主模型失败时自动切换到备用。'
  },
  {
    q: 'tenacity 的 retry 用于什么场景？',
    options: ['rate limiting', '自动retry失败请求', '成本计算', 'logging记录'],
    answer: 1,
    explain: 'retry机制提升服务可用性。'
  }
];
</script>
