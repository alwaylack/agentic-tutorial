# 第 23 章 · 实战三：全栈 Agent 服务

> Chapter goals：
> - Master FastAPI + LangGraph 后端 Agent 服务的完整架构
> - 学会使用 AI SDK 的 `useChat` 构建流式聊天前端
> - Implement基于 Redis 的消息persistence与 DynamoDB 长时记忆
> - 完成 Docker containerization并deployment到 AWS Fargate
> - 接入 CloudWatch monitoring与 Langfuse observability

> 📌 本章综合了本课程的多个章节：**FastAPI 生产基础**（[FastAPI ch22](/fastapi/ch22)）、**AI SDK Provider 管理**（[AI SDK ch03](/ai-sdk/ch03)）、**LangGraph streaming output**（[Agent 工程 ch08](/agent/ch08)）、**observability**（Langfuse（即将发布））。如遇到不熟悉的知识点，可回看对应章节。
...
│   │   ├── tools.py           # 工具函数
│   │   ├── persistence.py     # Redis/DynamoDB persistence
│   │   └── config.py          # 配置管理
│   ├── tests/
│   ├── Dockerfile
│   └── requirements.txt
...
## 23.2 阶段一：后端 FastAPI + LangGraph Agent
...
### 23.2.3 LangGraph Agent 定义

**app/agent.py**

```python
"""LangGraph Agent 定义：包含tool calling与状态管理"""
...
class AgentState(TypedDict):
    ...
    tool_results: list[str]  # 记录tool calling结果用于上下文
...
def should_continue(state: AgentState) -> str:
    """conditional edge：判断是否继续调用工具或结束"""
...
```python
"""消息persistence：Redis 短期记忆 + DynamoDB 长期记忆"""
import json
import uuid
...
## 23.5 阶段四：Docker containerization

### 23.5.1 后端 Dockerfile
...
# Redis monitoring：http://localhost:8001（需要 redis-commander）
```

---

## 23.6 阶段五：deployment到 AWS Fargate
...
## 23.7 阶段六：monitoring与observability

### 23.7.1 CloudWatch Logs

**app/main.py** 中添加logging中间件：
...
- **后端**：FastAPI + LangGraph Agent，支持tool calling与conditional edge
- **前端**：React + useChat，Implement流式聊天界面
- **persistence**：Redis 短期记忆 + DynamoDB 长期记忆
- **deployment**：Docker containerization → ECR → Fargate → CloudFront
- **observability**：CloudWatch loggingmonitoring + Langfuse AI 追踪

> 📌 **关键设计模式**：
> 1. **分层架构**：API 层 → Agent 层 → 工具层 → persistence层
> 2. **状态隔离**：短期对话用 Redis，长期用户数据用 DynamoDB
> 3. **deployment自动化**：一键脚本从构建到deployment全流程
> 4. **observability优先**：Langfuse 追踪每个 Agent 决策
...
2. **Implement断点续传**：在前端添加"恢复对话"功能，从 DynamoDB 加载历史消息
3. **性能压测**：使用 Locust（参考 [Locust 教程](/locust/)）测试 Agent 服务的concurrency能力
...
const quiz = [
  ...
  },
  {
    ...
    options: ["实时streaming output", "interrupt recovery与状态persistence", "模型微调", "Token 压缩"],
    answer: 1,
    explain: "Checkpointer 在 LangGraph 中用于persistence图状态，支持interrupt recovery、时间旅行和跨会话记忆。"
  },
  {
    question: "AWS Fargate deployment的主要优势是什么？",
...
- [FastAPI 生产deployment](/fastapi/ch22) — Docker 与deployment
- [AI SDK Provider 管理](/ai-sdk/ch03) — Gateway 与自定义 Provider
- [LangGraph persistence](/agent/ch07) — Checkpointer 与 Thread
- [Langfuse observability](/agent-prod/)（即将发布） — 追踪与evaluation
