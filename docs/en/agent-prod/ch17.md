# 第 17 章 · SQS message queue与 DynamoDB

> Chapter goals：
> 1. Understand SQS Standard 与 FIFO 两种队列的区别及适用场景；
> 2. Master boto3 对 SQS 的核心操作：创建队列、发送/接收/删除消息；
> 3. Understand死信队列（DLQ）的作用与配置方式；
> 4. 学会用 DynamoDB persistence Agent 的状态检查点；
> 5. 能将 SQS 消费者与 DynamoDB 组合成一个完整的production-grade Agent Worker。

本章是生产deployment的关键一环。前面的章节（[FastAPI ch22](/fastapi/ch22) 讲了 Docker 基础）已经覆盖了containerization，这里我们讨论"容器之外"的基础设施——让 Agent 服务能够async、可靠、水平扩展地运行。

---

## 17.1 SQS 标准队列与 FIFO 队列

Amazon Simple Queue Service（SQS）是最简单的 AWS 托管message queue。Agent 系统用 SQS 的典型场景是：**HTTP 接口接收用户请求 → 把任务塞进队列 → 后台 Worker async处理 → 结果写回数据库**。
...
| 适用场景 | 大量无关任务（如logging处理、通知） | 有顺序要求的任务（如工单状态流转） |
| 队列名后缀 | 无前缀 | 必须以 `.fifo` 结尾 |

> 💡 **选型原则**：Agent 任务是"独立且可retry"的 → Standard；Agent 需要严格按顺序处理（如同一用户的多个操作不能乱序）→ FIFO。
...
for msg in messages:
    ...
    print(f"  已被retry次数: {msg['Attributes']['ApproximateReceiveCount']}")
...
消息处理反复失败时，如果不加以控制，它会无限次重新入队，占用队列并掩盖真正的问题。**死信队列（DLQ）** 是一个专门的"墓地队列"——当源队列中的消息retry超过阈值后，SQS 自动把它移到 DLQ，源队列不再尝试投递。
...
for msg in dlq_messages.get('Messages', []):
    ...
    print(f"DLQ 卡住的 tasks: type={body.get('task')} | "
          f"原始队列retry了 {msg['Attributes']['ApproximateReceiveCount']} 次")
...
> 💡 **最佳实践**：任何生产队列都必须配 DLQ。否则"卡住的消息"会淹没你的monitoring和logging。

---

## 17.4 DynamoDB：Agent 状态存储

Agent 系统在运行时需要persistence状态：某个会话（thread）走到哪一步了、上次的推理结果是什么、有没有正在进行的tool calling……这些信息存在 SQLite 里不适合生产，存内存里一重启就丢。DynamoDB 是 AWS 托管的 NoSQL 键值库，**毫秒级延迟、无限扩展**，天然适合当 Agent 的"大脑记忆"。
...
| `type` | `checkpoint` / `result` | Distinguish状态快照与最终结果 |
...
def save_checkpoint(thread_id: str, step: int, state: dict) -> None:
    """把 Agent 当前状态persistence到 DynamoDB"""
    item = {
...
`ConditionalCheckFailedException` 不是错误——是**正常的concurrency保护机制**，业务代码应按预期处理（retry或放弃）。

---

## 17.5 完整示例：SQS 驱动 + DynamoDB persistence的 Agent Worker

把上面两部分合在一起，就是一个production-grade Agent Worker 的骨架：

```python
"""
agent_worker.py
SQS 消息驱动 + DynamoDB 状态persistence的 Agent Worker
"""
import json
import logging
import boto3
...
前端通过 [FastAPI](/fastapi/) 暴露一个 HTTP 接口把任务丢进 SQS，Worker 在另一个容器/进程中async消费：
...
> 🔗 deployment细节（Dockerfile、多容器编排、进程管理）参见 [FastAPI ch22](/fastapi/ch22)。
...
- **条件写入（`ConditionExpression`）**是处理concurrency更新的唯一安全方式，不要用"先 get 再 put"的两步模式；
- **production-grade架构**通常是：HTTP 接口（FastAPI）→ SQS → 后台 Worker（DynamoDB persistence结果）→ 前端轮询或 WebSocket 推送。
...
2. 给上面的 Worker 加一个 `retry_with_backoff` 装饰器：处理失败时等待 `2^attempt` 秒再重新入队，最多retry 3 次，超过 3 次自动移入 DLQ（通过调 `change_message_visibility` 把 VisibilityTimeout 设成 0 Implement）。
3. 在 DynamoDB 里Implement"检查点历史"功能：每次保存 checkpoint 时同时写入 `history#0`、`history#1`……，然后用 Query + `begins_with(sk, 'history#')` 查询某个 thread 的完整执行历史。
...
const quiz = [
  ...
  },
  {
    ...
    answer: 1,
    explain: '不删除消息会在 VisibilityTimeout 到期后对其他消费者重新可见，Implement自动retry。delete_message 必须在成功处理后调用。'
  },
  {
    q: 'DynamoDB 条件写入 ConditionExpression 的主要作用是什么？',
    options: [
      '提高写入性能',
      '防止concurrency Worker 覆盖彼此的 Agent 状态（乐观锁）',
      '自动加密存储的数据',
      '限制表的存储空间'
    ],
    answer: 1,
