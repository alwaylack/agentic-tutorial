...
> Chapter goals：
> - Master LangGraph `StateGraph` 多 Agent collaboration编排模式
> - 学会用conditional edge按工单类型路由到不同处理 Agent
> - Implement严重工单的人工审批（Human-in-the-loop）
> - 接入 Celery Worker Implementasync任务队列
> - 构建完整的工单全生命周期处理系统

本章是「实战二」：我们在实战一（RAG 知识库 Agent）的基础上，构建一个**多 Agent collaboration的工单处理系统**。系统将涵盖工单从创建、分类、分析、解决到审核的全流程，并支持asyncmessage queue和人机collaboration。

本系统包含四个 Agent：
- **接收 Agent**：负责工单分类和优先级evaluation
...
> 💡 状态设计要点：
> - `messages` 字段使用 `operator.add` Implement消息累积
> - `needs_review` 和 `approved` 支持interrupt recovery模式
...
if __name__ == "__main__":
    ...
    print(f"审计logging: {result['audit_log']}")
...
## 22.3 Implement Human-in-the-loop 审批
...
def process_ticket_async(self, ticket_data: dict):
    ...
    try:
        ...
        # 自动retry
        print(f"[Worker] 工单处理失败，将在 60 秒后retry: {exc}")
...
async def create_ticket(request: TicketRequest, background_tasks: BackgroundTasks):
    ...
    return TicketResponse(
        ticket_id=ticket_id,
        status="queued",
        message=f"工单已提交，async处理中 (task_id: {task.id})"
    )
...
2. **conditional edge路由**：根据优先级和状态动态路由到不同节点
3. **Human-in-the-loop**：使用 `interrupt()` Implement人工审批节点
4. **检查点与续跑**：支持中断后的状态恢复和人工决策续跑
5. **Celery async队列**：Implement工单async处理和批量处理
...
| `MemorySaver` | 检查点persistence | 演示代码 |
| `@celery_app.task` | async任务装饰器 | `worker.py` |

## 🛠️ 动手实践

1. **扩展工单类型**：添加更多分类规则（如安全漏洞、性能问题），并设计对应的处理流程
2. **Implementpersistence存储**：将工单数据存入 SQLite 或 PostgreSQL，Implement跨重启的状态恢复
...
const quiz = [
  {
    question: "在 LangGraph 中，如何定义state graph？",
    options: [
  ...
  {
    question: "以下哪个方法用于Implement Human-in-the-loop？",
    options: [
      "workflow.interrupt()",
      "interrupt()",
      "workflow.pause()",
  ...
  },
  {
    ...
    answer: 2,
    explanation: "示例中使用了 max_retries=3 参数，配合 retry() 方法Implement自动retry，超过上限后标记失败。"
  },
  {
    question: "conditional edge route_by_priority 返回的值会用作什么？",
    options: [
      "工单优先级编号",
      "下一个节点的名称",
      "审批意见",
    ...
    answer: 1,
    explanation: "add_conditional_edges 的 edge_fn 返回值对应conditional edge映射中的 key，用于选择下一个要执行的节点。"
  }
]
</script>
