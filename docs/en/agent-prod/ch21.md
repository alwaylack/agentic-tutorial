# 第 21 章 · 实战一：RAG 知识库 Agent

> Chapter goals：
> - 从零构建一个完整的 RAG Agent 服务
> - 使用 Vercel AI SDK 的嵌入向量检索文本
> - 用 LangGraph StateGraph 编排检索 + 生成流程
> - 通过 Checkpointer Implement多轮对话persistence
> - 用 FastAPI 暴露 HTTP 接口
> - 接入 Langfuse Implement全链路追踪

本章将逐步构建一个production-grade RAG Agent。我们从最基础的语义检索开始，逐步加入 LangGraph state graph、对话persistence、API 接口和observability。每阶段都是完整可运行的代码。

> 📌 **前置知识**：本章需要Understand [Vercel AI SDK 嵌入向量](/ai-sdk/ch11) 和 [LangGraph 基础概念](./ch04)。

## 21.1 阶段一：基础 RAG — 嵌入检索
...
documents = [
    ...
    ("LangGraph 是 LangChain 团队开发的图结构 Agent 框架，支持状态管理、persistence和人机collaboration。它是构建复杂 Agent 工作流的首选工具。", {"source": "技术文档"}),
...
## 21.2 阶段二：LangGraph StateGraph 编排
...
## 21.3 阶段三：persistence对话历史

state graph需要 Checkpointer 才能在多轮对话中保持上下文。LangGraph 支持多种 Checkpointer，我们先使用内存版，然后展示 PostgreSQL 版。

### 21.3.1 内存 Checkpointer
...
### 21.3.2 PostgreSQL Checkpointer（生产环境）
...
## 21.4 阶段四：FastAPI HTTP 接口

将 Agent 封装为 REST API，支持streaming output。
...
## 21.5 阶段五：Langfuse observability

接入 Langfuse Implement全链路追踪，monitoring Token 使用、延迟和成本。
...
### 21.5.3 Langfuse Dashboard

deployment后，你可以在 Langfuse Dashboard 看到：
...
本章完整Implement了production-grade RAG Agent 服务：

- **阶段一**：基础 RAG 引擎，使用嵌入向量进行语义检索
- **阶段二**：LangGraph StateGraph 编排检索与生成逻辑
- **阶段三**：Checkpointer Implement多轮对话persistence
- **阶段四**：FastAPI 暴露 HTTP 接口，支持streaming output
- **阶段五**：Langfuse 全链路observability

### 关键知识点

1. `createOpenAICompatible` 可接入任何 OpenAI 兼容的嵌入模型
2. `StateGraph` 的conditional edgeImplement动态路由
...
3. **Implementinterrupt recovery**：在 `generate_node` 前加入 `interrupt()`，Implement人机collaboration的审批流程
...
const quiz = [
  {
    q: "在 LangGraph 中，如何Implement在生成节点前先检索知识库？",
    options: [
      "A. 在 StateGraph 中添加 retrieve_node 和 generate_node，并用conditional edge连接",
      "B. 使用 parallel_node 同时执行两个节点",
      "C. 通过 tool_call 触发检索",
      "D. 在 END 节点前调用外部服务"
    ],
    answer: 0,
    explain: "LangGraph 的核心模式是定义节点并按边连接。retrieve → generate 的顺序执行通过 add_edge 或conditional edgeImplement。"
  },
  {
    q: "MemorySaver 和 PostgresSaver 的主要区别是什么？",
    options: [
      "A. MemorySaver 用于开发，PostgresSaver 用于生产persistence",
      "B. MemorySaver 支持streaming output，PostgresSaver 不支持",
      "C. PostgresSaver 更快但 MemorySaver 更安全",
      "D. 两者没有区别，可以互换使用"
    ],
    answer: 0,
    explain: "MemorySaver 将状态存储在内存中，进程重启后丢失；PostgresSaver 使用 PostgreSQL persistence，适合生产环境。"
  },
  {
    ...
    answer: 0,
    explain: "Langfuse 是observability平台，@observe 自动捕获执行指标，便于追踪和分析 Agent 行为。"
  },
  {
