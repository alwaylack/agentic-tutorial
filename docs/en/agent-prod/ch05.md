# 第 5 章 · 状态管理与tool calling

> Chapter goals：
> - Master MessagesState 的消息结构
> - 学会定义自定义状态（TypedDict 扩展）
> - Understand @tool 装饰器与 ToolNode
> - Implement完整的 Agent 图（LLM 节点 + ToolNode）
...
- `ToolNode` 自动处理tool calling和结果回写
- conditional edgeImplementtool calling的循环逻辑

## 🛠️ 动手实践

1. 定义两个自定义工具（计算器、日期查询），组装成 Agent 图
2. Implementtool calling失败时的错误处理（返回错误消息给 LLM）
...
const quiz = [
  {
    q: 'MessagesState 的核心字段是什么？',
    options: ['data', 'messages', 'context', 'output'],
    answer: 1,
    explain: 'MessagesState 包含 messages 列表，存储对话历史。'
  },
  {
    q: 'ToolNode 的作用是什么？',
    options: ['定义工具', '自动处理tool calling和结果', '管理状态', '编译图'],
    answer: 1,
    explain: 'ToolNode 自动解析 tool_calls、调用工具、包装 ToolMessage。'
  },
  {
    q: 'conditional edge的返回值决定什么？',
    options: ['节点名称', '下一个执行的节点', '状态更新', '错误处理'],
    answer: 1,
    explain: 'conditional edge函数返回字符串，匹配对应的边目标节点。'
  },
  {
    q: 'operator.add 在状态字段中的作用是什么？',
    options: ['覆盖旧值', '追加新值', '删除值', '排序'],
    answer: 1,
    explain: 'Annotated[..., operator.add] 使该字段在状态更新时追加而非覆盖。'
  }
];
</script>
