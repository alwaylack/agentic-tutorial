...
> - 用 OpenCV Implement「场景切分」与关键帧提取，把长视频压缩成可检索的图像集
...
固定帧率抽帧会产生大量冗余画面。**场景切分**只在画面内容显著变化时取帧，是视频Understand的正确入口。
...
> ⚠️ **渲染管线三坑**：① `networkidle` 在长轮询页面会永远等不到，务必设 timeout 兜底；② 无头浏览器内存占用高（每实例 200MB+），concurrency要rate limiting；③ 只渲染可信 URL——恶意页面可能利用浏览器漏洞，生产环境必须跑在沙箱容器里（见 20.5）。
...
2. **超时与retry**：每个 FFmpeg 命令设 timeout（本节 `run_ffmpeg` 已内置），配合 ch01 的指数退避retry；转录失败降级到更小的 Whisper 模型。
...
- **生产四件套**：独立 worker 队列、超时retry降级、临时文件治理、成本上报

## 🛠️ 动手实践

1. **会议视频知识入库**：把 `process_video` 的输出（转录文本 + 关键帧描述）接入第 19 章扩展一的 `build_ingest_graph`，Implement「上传会议录像 → 自动进知识库」的完整链路。
2. **媒体处理压测**：用 Locust（复用压测课程 ch13 协议扩展思路）对媒体上传接口压测，验证 Celery `media` 队列在 10 concurrency下的积压与 Worker 扩容策略。
3. **截图 diff monitoring**：用 Playwright 每小时渲染同一个仪表盘页面，对截图做像素 diff（OpenCV `absdiff`），变化超阈值时发 CloudWatch 告警——一个零成本的 UI 巡检机器人。
