# 第 12 章 · LLM-as-judge evaluation器

> Chapter goals：
> - Understand LLM-as-judge 的core concepts与适用场景
> - Master Judge 提示词设计：rubric + few-shot examples
> - 学会批量evaluation脚本的编写与执行
> - 建立自动化evaluation管线：generate → judge → 统计
> - 实战：evaluation RAG 回答的准确性（faithfulness + relevance）

## 12.1 什么是 LLM-as-judge

在构建和迭代 Agent 系统时，一个核心问题是：**如何知道你的 Agent 是否"做得好"？**

传统做法是人工审查输出——找 10 条测试数据，逐条判断质量。这在数据量少时可行，但当你的 Agent 每天处理数千条请求、或者你需要反复迭代提示词时，人工evaluation成为瓶颈。

**LLM-as-judge** 的思路很简单：让另一个 LLM 来evaluation第一个 LLM 的输出质量。
...
LLM-as-judge 的优势在于**速度快、成本低、可批量执行**。一个 GPT-4o-mini 作为 judge，每次调用成本不到 $0.001，可以在几秒内完成数百条数据的evaluation。
...
| RAG 回答准确性 | ⭐⭐⭐ | 可evaluation faithfulness + relevance |
| 代码生成质量 | ⭐⭐ | 可evaluation正确性、可读性 |
| 分类/抽取任务 | ⭐ | 有客观答案，用精确匹配更可靠 |
| 数学计算 | ✗ | 用解析解验证，不要用 judge |

### 关键认知：Judge 本身也会犯错

LLM-as-judge 不是完美的——judge 模型有自己的偏差、会"过于严格"或"过于宽松"。但研究表明，当 judge 和被evaluation的是同代或更代模型时，evaluation质量足够可靠。
...
3. **示例（Few-shot）**：让 judge Understand标准的具体表现
...
✅ 好的 rubric（RAG faithfulness evaluation）：
```
- 2分：回答完全基于参考文本，无额外信息
...
有了 judge prompt，下一步是编写批量evaluation脚本。核心逻辑：
...
class RagJudgePrompt:
    """RAG 回答evaluation的 Judge prompt 模板"""
    
    SYSTEM_PROMPT = """\
你是一个专业的 RAG 系统evaluation员。你的任务是evaluation AI 助手的回答质量。

evaluation维度：Faithfulness（忠实度）
...
## 12.4 自动化evaluation管线

完整的evaluation管线包含三个阶段：
    ...
    test_cases = [
        TestCase(
            ...
            reference_context="LangGraph 是 LangChain 团队开发的图结构工作流引擎，"
                           ...
                           "它基于 StateGraph API，支持tool calling、subgraph组合和persistence。"
            ),
        TestCase(
...
## 12.5 实战：evaluation RAG 回答质量

下面是一个完整的端到端示例，evaluation一个简易 RAG 系统的回答质量。
...
TEST_CASES = [
    {
        ...
        "context": "LangGraph 是 LangChain 开发的图结构 Agent 编排引擎。"
                  ...
                  "支持tool calling（ToolNode）、条件路由（conditional edges）和循环结构。",
  ...
  "results": [
    {
      "question": "LangGraph 是什么？",
      "score": 2,
      "explanation": "回答完全基于参考文本，准确描述了 LangGraph 的core concepts。"
    },
    {
      "question": "Deep Agents 和 LangGraph 有什么区别？",
      "score": 1,
...
## 12.6 进阶：多维度evaluation

生产环境的evaluation往往需要多维度。以 RAG 为例，常见维度包括：
...
class MultiDimensionalJudge:
    ...
    SYSTEM_PROMPT = """\
你是一个专业的 RAG 系统evaluation员。请从以下维度evaluation AI 回答：
...
- **[FastAPI](/fastapi/)** ch22：Docker deployment，可将evaluation服务containerization

## 本章小结

- **LLM-as-judge**：用 LLM evaluation LLM 输出，速度快成本低，适合开放式文本质量判断
- **Judge prompt 设计三要素**：角色定义 + 量化 rubric + few-shot 示例
- **evaluation管线三步走**：Generate → Judge → Stats，可完全自动化
- **RAG evaluation常用维度**：Faithfulness（忠实度）+ Relevance（相关性）+ Completeness（完整性）
- **关键认知**：Judge 本身也会犯错，保留人工抽检和交叉验证


## 🛠️ 动手实践

1. 修改 `judge.py` 中的 `RagJudgePrompt`，增加 **Hallucination** 维度（回答是否包含参考文本外的内容），重新运行evaluation并观察分数变化。
2. 扩展测试数据集至 10 条，运行批量evaluation，分析平均分数分布，找出得分最低的测试用例并改进提示词。
3. 将evaluation脚本改造为 FastAPI 接口，接受 `{question, context}` 输入，返回evaluation结果 JSON，参考 [FastAPI](/fastapi/) ch01-06。
...
const quiz = [
  {
    ...
    answer: 1,
    explain: "LLM-as-judge 擅长主观性较强的开放式文本evaluation（如质量、相关性），而数学计算、分类等有明显客观标准的任务更适合用规则验证。"
  },
  {
  ...
  {
    question: "批量evaluation管线中，生成回答和 judge evaluation的关系是？",
    options: [
      "必须先人工审查所有回答",
    ...
    answer: 1,
    explain: "标准evaluation管线是：先生成回答（Step 1），再用 judge evaluation（Step 2），最后统计（Step 3）。这是完全自动化的流程。"
  },
  {
    question: "evaluation RAG 回答时，'Faithfulness' 维度的 0 分标准是？",
    options: [
      "回答与参考文本完全一致",
      "回答基于参考文本但有少量推断",
      "回答与参考文本无关或包含错误",
...
上一章：context engineering（即将发布） | 下一章：A/B 测试与对照实验（即将发布）
[上一章：context engineering](/agent-prod/ch11) | [下一章：A/B 测试与对照实验](/agent-prod/ch13)
